...
import { BoxIcon, DeliveredIcon, DeliveryLineIcon, DispatchIcon, TimesIcon, WhiteBoxIcon, WhiteDeliveredIcon, WhiteDispatchIcon } from '../components/SVGs/SVGicons'; // Added CheckedIcon
import { StoreOrderResponse } from '../components/models/ISellerStore';
import { useUpdateDeliveryStatus } from '../api/apiClients';
import { DeliveryStatus, StatusEnum } from '../components/models/IOrderDeliveryStatus';
...
const SellerDeliveryStatus = ({ visibility, setVisibility, selectedOrder }: Props) => {
    ...
    useEffect(() => {
        if (selectedOrder) {
            const storedStatuses = localStorage.getItem(`selectedStatuses-${selectedOrder.id}`);
            if (storedStatuses) {
                setSelectedStatuses(JSON.parse(storedStatuses));
            } else {
                // Default to existing status from selectedOrder if no localStorage data
                const statusForStore = selectedOrder.status.find(s => s.storeId === selectedOrder.id);
                if (statusForStore) {
    ...
    async function handleUpdateStatus(e: FormEvent<HTMLButtonElement>, status: StatusEnum) {
        e.preventDefault();
        setIsUpdating(true);
        ...
        try {
            const response = await updateStatus(selectedOrder?.id as string, data);
            // console.log(response.data.data);
            setSelectedStatuses(prevStatuses => {
                const newStatuses = [...prevStatuses, status];
                localStorage.setItem(`selectedStatuses-${selectedOrder?.id}`, JSON.stringify(newStatuses));
                return newStatuses;
            });
        } catch (err) {
            console.log(err);
    ...
    const isStatusSelected = (status: StatusEnum) => selectedStatuses.includes(status);
        ...
        >
            <div className="bg-white w-[90%] mx-auto md:w-full rounded-[34px] p-7">
                <div className="flex flex-col gap-3 mb-10">
                    <span onClick={() => setVisibility(false)} className='cursor-pointer flex items-end justify-end ml-auto w-fit'>
                        <TimesIcon />
                    </span>
                ...
                <div className="flex items-center gap-7 w-full mb-5 overflow-x-auto whitespace-nowrap">
                    <div className="text-center flex flex-col gap-3">
                        <div className="flex items-center gap-5">
                            <button
                                ...
                                ${isStatusSelected(StatusEnum.Pending) && '!bg-[#2C7865]'} 
                                hover:bg-[#D9EDBF]`}
                                disabled={isUpdating}
                            >
                                {isStatusSelected(StatusEnum.Pending) ? <WhiteBoxIcon /> : <BoxIcon />}
                            </button>
                    ...
                    <div className="text-center flex flex-col gap-3">
                        <div className="flex items-center gap-5">
                            <button
                                ...
                                className={`w-[90px] h-[90px] rounded-full flex items-center justify-center cursor-pointer bg-[#E7E7E7]
                                ${isStatusSelected(StatusEnum.Dispatched) && '!bg-[#2C7865]'} 
                                hover:bg-[#D9EDBF]`}
                                disabled={isUpdating}
                            >
                                {isStatusSelected(StatusEnum.Dispatched) ? <WhiteDispatchIcon /> : <DispatchIcon />}
                            </button>
                            <span><DeliveryLineIcon /></span>
                        </div>
                        <div className="flex flex-col items-start">
                    ...
                    <div className="text-start flex flex-col gap-3">
                        <button
                            ...
                            className={`w-[90px] h-[90px] rounded-full flex items-center justify-center cursor-pointer bg-[#E7E7E7]
                            ${isStatusSelected(StatusEnum.Delivered) && '!bg-[#2C7865]'} 
                            hover:bg-[#D9EDBF]`}
                            disabled={isUpdating}
                        >
                            {isStatusSelected(StatusEnum.Delivered) ? <WhiteDeliveredIcon /> : <DeliveredIcon />}
                        </button>
                        <p>Order delivered</p>
                        <span>23-4-2024</span>
                    </div>
