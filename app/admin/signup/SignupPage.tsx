...
import { useRegisterAdmin, useRegisterSeller } from '@/app/api/apiClients'
...
import { RegisterAdminRequest } from '@/app/components/models/IRegisterAdmin'
...
const SignupPage = (props: Props) => {
    ...
    const [formValues, setFormValues] = useState<RegisterAdminRequest>();
    ...
    const [companyTokenErrorMsg, setCompanyTokenErrorMsg] = useState<string | boolean>(false);
    ...
    function onformValueChange(
        ...
        setFormValues({
            ...(formValues as RegisterAdminRequest),
            [name]: value,
        });
    ...
    function validateFields() {
        ...
        if (
            ...
            if (!formValues?.companyToken) {
                setCompanyTokenErrorMsg(true);
            } else {
                setCompanyTokenErrorMsg(false);
            }
    ...
    async function handleFormSubmission(e: FormEvent<HTMLFormElement>) {
        ...
        if (validateFields()) {
            ...
            await registerAdmin(formValues as RegisterAdminRequest)
                .then((response) => {
        ...
        <div className={styles.fieldContainer}>
          <div className={styles.rowForm}>
            <div className={styles.formField}>
              ...
              />
              {fullNameErrorMsg && (
                <span className={styles.errorMsg}>Please enter full name</span>
              )}
            </div>
            <div className={styles.formField}>
              <label htmlFor="companyToken">
                <span>*</span>Company Token
              </label>
              <input
                type="text"
                name="companyToken"
                id="companyToken"
                placeholder="Please enter your company token "
                value={formValues?.companyToken}
                onChange={(e) => onformValueChange(e, setCompanyTokenErrorMsg)}
              />
              {companyTokenErrorMsg && (
                <span className={styles.errorMsg}>
                  Please enter company token
                </span>
              )}
            </div>
          </div>
          <div className={styles.rowForm}>
            ...
            <div className={styles.formField}>
              ...
              {cofirmPasswordErrorMsg && (
                <span className={styles.errorMsg}>Please confirm password</span>
              )}
            </div>
          </div>
          <div className={styles.rowForm}>
            <div className={styles.formField}>
              ...
              <input
                ...
                onChange={(e) => onformValueChange(e, setCountryErrorMsg)}
              />
              {countryErrorMsg && (
                <span className={styles.errorMsg}>Please enter country</span>
              )}
            </div>
            <div className={styles.formField}>
          ...
          <div className="flex flex-col mb-8 gap-2">
            <div className={styles.acknowledge}>
              ...
              <label htmlFor="acknowledge">
                I agree to the <Link href={"/"}>Terms of Use</Link>, and{" "}
                <Link href={"/"}>Privacy Policy</Link>. I agree to receive more
                information from Rayvvin about its products and services.
              </label>
            </div>
            {checkboxErrorMsg && (
              <span className={styles.errorMsg}>{checkboxErrorMsg}</span>
            )}
          </div>
          <button
            type="submit"
