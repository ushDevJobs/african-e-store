...
export function GenerateOrdersFunction(payload: any) {
  /**
   * @returns the response for the api request
   */
  async function generateOrders(data: any) {
    //Fetch message
    const response = await API.post(ApiRoutes.GenerateOrders, data);

    //Return response
    return response;
  }
  return generateOrders(payload);
}
...
export function useAdminFetchUsers() {
  async function adminFetchUsers() {
    // Fire the request
    const response = await API.get(ApiRoutes.AdminFetchUsers);

    // Return the response
    return response;
  }

  return adminFetchUsers;
}
...
export function useUpdateDeliveryStatus() {
  async function updateStatus(id: string, data: DeliveryStatus) {
    ...
    const response = await API.patch(
      `${ApiRoutes.UpdateDeliveryStatus}/${id}`,
      data
    );
