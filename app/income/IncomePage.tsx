        ...
        <>
            {isFetchingIncome ? (
                ...
                <div className={`${styles.main} flex flex-col gap-20`}>
                    <Chart income={income} />
                    <div>
                        <h1 className="text-[#333333] text-2xl font-bold mb-10">
                            Transactions
                        </h1>
                            ...
                            {income.transactions.map((transaction, index) => (
                                <div className="flex gap-6" key={index}>
                                    <div className="bg-[#D9EDBF] text-[#2C7865] text-xl font-medium rounded-2xl flex flex-col gap-1 justify-center items-center min-w-[168px] h-[219px]">
                                        {transaction.orderDetails.length}{" "}
                                        {transaction.orderDetails.length > 1 ? "items" : "item"}
                                        <BoxIcon />
                                    </div>
                                    <div className="flex flex-col gap-2">
                                        {transaction.orderDetails.slice(0, 1).map((detail, index) => (
                                            <p key={index} className="text-[#6F6F6F] text-lg">
                                                {detail.product.name}
                                            </p>
                                        ))}
                                        {transaction.orderDetails.slice(0, 1).map((detail, index) => (
                                            <div className="flex flex-col gap-2" key={index}>
                                                <p>Item number {transaction.orderId}</p>
                                                <h3 className="text-[#000] font-medium text-sm">
                                                    {detail.status}
                                                </h3>
                                            </div>
                                        ))}
                                        <h3 className="font-medium text-xl text-[#1E1E1E]">
                                            &pound;
                                            {/* {transaction.orderDetails[0] &&
                        transaction.amount.toLocaleString()} */}
                                        ...
                                        <div className="flex items-center gap-5 text-[#6F6F6F] text-sm mb-6 mt-4 whitespace-nowrap">
                                            ...
                                            <p className="flex items-center gap-1">
                                                ...
                                                {transaction.user.address.city}, {transaction.user.address.country}
                                            </p>
                                            <p className="flex items-center gap-1">
                                                <span>
                                                    <CalenderIcon />
